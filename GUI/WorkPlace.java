...
public class WorkPlace extends JPanel {
    ...
    public void paintComponent(Graphics g) {
        ...
        if (!WorkEnvironment.focusmodding.hasnodata()) paintfocusmode(g2d);
        g2d.dispose();
    }
    private void paintfocusmode(Graphics2D g2d) {
        if (!(WorkEnvironment.focusmodding.trees.size() > WorkEnvironment.focusmodding.selectedtree)) {
            return;
        }
    ...
    private void drawAND(Graphics2D g2d, NationalFocus focusfrom, NationalFocus focusto) {
        ...
        int[] xyfrom = WorkEnvironment.focusmodding.trees.get(WorkEnvironment.focusmodding.selectedtree).focus_xy(focusfrom, new ArrayList<>(Collections.emptyList()));
        int[] xyto = WorkEnvironment.focusmodding.trees.get(WorkEnvironment.focusmodding.selectedtree).focus_xy(focusto, new ArrayList<>(Collections.emptyList()));
    ...
    private void paintfocuses(Graphics2D g2d) {
        for (NationalFocus focus : WorkEnvironment.focusmodding.trees.get(WorkEnvironment.focusmodding.selectedtree).nationalfocuses) {
            ...
            g2d.setColor(ColorList.BLACK[0]);
            int[] xy = WorkEnvironment.focusmodding.trees.get(WorkEnvironment.focusmodding.selectedtree).focus_xy(focus, new ArrayList<>(Collections.emptyList()));
            g2d.fillRect(xybound(xy[0], 40.0f) + 300, xybound(xy[1], 40.0f) + 300, 40, 40);
            g2d.drawString(focus.id, xy[0] * WorkEnvironment.scale * 40 + 300, xy[1] * WorkEnvironment.scale * 40 + 380);
        }
    }
    private void paintcontinuousfocus(Graphics2D g2d){
        g2d.setColor(new Color(0,0,0,50));
        Shape shape = new Rectangle(WorkEnvironment.focusmodding.trees.get(WorkEnvironment.focusmodding.selectedtree).continuous_focus_position_x, WorkEnvironment.focusmodding.trees.get(WorkEnvironment.focusmodding.selectedtree).continuous_focus_position_y, 200, 100);
        g2d.draw(shape);
    ...
    private int xybound(int pos, float size) {
        return Math.round(pos * WorkEnvironment.scale * size);
    }
}